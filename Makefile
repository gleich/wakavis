...
build-docker-prod:
	docker build -t mattgleich/wakavis:latest .
build-docker-dev:
	docker build -f dev.Dockerfile -t mattgleich/wakavis:test .
build-docker-dev-lint:
	docker build -f dev.lint.Dockerfile -t mattgleich/wakavis:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm wakavis
